...
2. Download `Ponten-1.2.10.dmg` (for Mac) or `Ponten-Setup-1.2.10.exe` (for Windows installer).
3. **Mac**: Open the downloaded file and drag the **Ponten** icon into the **Applications** folder.
4. **Windows**: Run `Ponten-Setup-1.2.10.exe` to install the app. It will create a Start Menu shortcut, register the uninstaller, and quietly launch in your System Tray. *(A portable `.exe` is also available if you prefer no installation).*
